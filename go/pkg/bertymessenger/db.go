...
func (d *dbWrapper) addConversationForContact(groupPK, contactPK string) (*Conversation, error) {
	...
	if err := d.tx(func(d *dbWrapper) error {
...
func (d *dbWrapper) getAllInteractions() ([]*Interaction, error) {
	...
	return interactions, d.db.Find(&interactions).Error
}
...
func (d *dbWrapper) addContactRequestIncomingAccepted(contactPK, groupPK string) (*Contact, *Conversation, error) {
	...
	if err := d.db.Transaction(func(db *gorm.DB) error {
		...
		if err := tx.First(&contact).Error; err != nil {
			return err
		}
...
func (d *dbWrapper) markInteractionAsAcknowledged(cid string) (bool, error) {
	...
	if err := d.tx(func(db *dbWrapper) error {
		...
		tx = d.db.Model(&Interaction{}).Where(map[string]interface{}{"cid": cid, "acknowledged": false}).Update("acknowledged", true)
...
func (d *dbWrapper) addInteraction(i Interaction) (*Interaction, error) {
	...
	if err := d.db.First(&ret, &Interaction{CID: i.CID}).Error; err != nil {
		return nil, err
	}
...
func (d *dbWrapper) attributeBacklogInteractions(devicePK, groupPK, memberPK string) ([]*Interaction, error) {
	...
	if err := d.tx(func(d *dbWrapper) error {
		...
		if err := tx.Update("member_public_key", memberPK).Error; err != nil {
			return err
		}

		if err := d.db.Order("ROWID asc").Find(&backlog, cids).Error; err != nil {
			return err
		}
...
func (d *dbWrapper) addMember(memberPK, groupPK, displayName string) (*Member, error) {
	...
	if err := d.tx(func(d *dbWrapper) error {
		...
		return d.db.Clauses(&onConflict).Create(&member).Error
	}); err != nil {
		return nil, err
	}
...
func (d *dbWrapper) setConversationIsOpenStatus(conversationPK string, status bool) (bool, error) {
	...
	if err := d.tx(func(wrapper *dbWrapper) error {
		values := map[string]interface{}{
			"is_open": status,
		}
		...
		if count != 1 {
			return errcode.ErrInvalidInput.Wrap(fmt.Errorf("invalid conversation public key"))
		}
...
func (d *dbWrapper) addServiceToken(accountPK string, serviceToken *bertytypes.ServiceToken) error {
	...
	if err := d.tx(func(db *dbWrapper) error {
		for _, s := range serviceToken.SupportedServices {
			tx := db.db.Clauses(clause.OnConflict{DoNothing: true}).Create(&ServiceToken{
				AccountPK:         accountPK,
