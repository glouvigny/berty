...
func handleProtocolEvent(svc *service, gme *bertytypes.GroupMetadataEvent) error {
	...
	handler, ok := map[bertytypes.EventType]func(svc *service, gme *bertytypes.GroupMetadataEvent) error{
		...
		bertytypes.EventTypeAccountServiceTokenAdded:              accountServiceTokenAdded,
	}[et]
...
func accountServiceTokenAdded(svc *service, gme *bertytypes.GroupMetadataEvent) error {
	...
	if err := svc.db.addServiceToken(bytesToString(config.AccountPK), ev.ServiceToken); err != nil {
...
func accountGroupJoined(svc *service, gme *bertytypes.GroupMetadataEvent) error {
	...
	case errcode.Is(err, errcode.ErrDBEntryAlreadyExists):
		svc.logger.Info("conversation already in db")
	case err != nil:
	...
	svc.logger.Info("AccountGroupJoined", zap.String("pk", groupPK), zap.String("known-as", conversation.GetDisplayName()))
...
func handleAppMessageUserMessage(svc *service, tx *dbWrapper, i *Interaction, amPayload proto.Message) (*Interaction, error) {
	...
	if isOpened, err := svc.db.isConversationOpened(i.ConversationPublicKey); err != nil {
		return i, err
	} else if !isOpened {
...
func handleAppMessage(svc *service, gpk string, gme *bertytypes.GroupMessageEvent, am *AppMessage) error {
	...
	handler, ok := map[AppMessage_Type]struct {
