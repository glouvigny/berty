...
func Test_dbWrapper_markInteractionAsAcknowledged(t *testing.T) {
	...
	updated, err := db.markInteractionAsAcknowledged("QmXXXX")
	require.Error(t, err)
	require.False(t, updated)

	db.db.Create(&Interaction{CID: "Qm0001", Acknowledged: true})
	updated, err = db.markInteractionAsAcknowledged("Qm0001")
	require.NoError(t, err)
	require.False(t, updated)

	db.db.Create(&Interaction{CID: "Qm0002", Acknowledged: false})
	updated, err = db.markInteractionAsAcknowledged("Qm0002")
	require.NoError(t, err)
...
func Test_dbWrapper_setConversationIsOpenStatus(t *testing.T) {
	...
	updated, err := db.setConversationIsOpenStatus("conv_xxx", true)
	require.Error(t, err)
	require.False(t, updated)
	...
	updated, err = db.setConversationIsOpenStatus("conv1", true)
	require.NoError(t, err)
	require.False(t, updated)
	...
	updated, err = db.setConversationIsOpenStatus("conv1", false)
	require.NoError(t, err)
	require.True(t, updated)
	...
	updated, err = db.setConversationIsOpenStatus("conv2", true)
	require.NoError(t, err)
	require.True(t, updated)
...
func Test_dbWrapper_tx(t *testing.T) {
	...
	err := db.tx(func(db *dbWrapper) error {
	...
	err = db.tx(func(db *dbWrapper) error {
		err := db.addAccount("some pk", "some url")
		require.NoError(t, err)
		return nil
	})
	require.NoError(t, err)
	...
	err = db.tx(func(db *dbWrapper) error {
		err := db.addAccount("some pk 2", "some url 2")
		require.NoError(t, err)
		return fmt.Errorf("some error")
	})
	require.Error(t, err)
